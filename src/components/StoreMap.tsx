...
import { HiMapPin, HiPhone, HiClock, HiClipboardDocument, HiBuildingStorefront } from 'react-icons/hi2';
...
const StoreMap: React.FC<StoreMapProps> = ({ className = '' }) => {
  ...
  const openGoogleMaps = (storeIndex: number = selectedStore) => {
    ...
    const address = encodeURIComponent(`${store.street}, ${store.neighborhood}, ${store.city} - ${store.state}, ${store.zipCode}`);
  ...
  return (
    <div className={`bg-white rounded-xl shadow-2xl overflow-hidden border border-gray-100 ${className}`}>
      {/* Seletor de Lojas */}
      ...
      <div className="relative h-80 bg-gradient-to-br from-orange-100 to-red-100">
        <iframe
          src={`https://www.openstreetmap.org/export/embed.html?bbox=${currentStore.coordinates.lng-0.01},${currentStore.coordinates.lat-0.01},${currentStore.coordinates.lng+0.01},${currentStore.coordinates.lat+0.01}&layer=mapnik&marker=${currentStore.coordinates.lat},${currentStore.coordinates.lng}`}
        ...
        <div className="absolute bottom-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-2 rounded-lg shadow-lg">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm font-medium text-gray-800">{currentStore.name}</span>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-gray-600 leading-relaxed">
                ...
                {currentStore.neighborhood}, {currentStore.city} - {currentStore.state}<br />
              ...
              >
                ...
                <span>{copiedAddress === selectedStore ? 'Endereço copiado!' : 'Copiar endereço'}</span>
              </button>
            </div>
          </div>
        </div>
              ...
              <div className="space-y-2">
                {businessHours.map((schedule, index) => (
                  <div key={index} className="flex justify-between text-sm">
                    ...
                    <span className={`font-medium ${schedule.hours === 'Fechado' ? 'text-red-500' : 'text-gray-800'}`}>
                      {schedule.hours}
                    </span>
                  </div>
                ))}
