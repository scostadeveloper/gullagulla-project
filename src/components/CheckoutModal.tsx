...
const CheckoutModal: React.FC<CheckoutModalProps> = ({ isOpen, onClose }) => {
  ...
  const [step, setStep] = useState<"info" | "payment" | "pix" | "success">(
    "info"
  );
  const [paymentMethod, setPaymentMethod] = useState<
    "pix" | "delivery" | "credit" | "debit" | "pickup"
  >("pix");
  ...
  useEffect(() => {
    if (isOpen) {
      const savedCustomerInfo = localStorage.getItem(
        "gullagulla-customer-info"
      );
  ...
  const generatePixPayment = () => {
    ...
    const pixData = {
      qrCode: `00020126330014BR.GOV.BCB.PIX011136995827000520400005303986540${finalValue.toFixed(
        2
      )}5802BR5925GULLA GULLA SALGADOS6009SAO PAULO62070503***6304`,
      copyPaste: `00020126330014BR.GOV.BCB.PIX011136995827000520400005303986540${finalValue.toFixed(
        2
      )}5802BR5925GULLA GULLA SALGADOS6009SAO PAULO62070503***6304`,
  ...
  const handleSubmitPayment = async () => {
    ...
    try {
      ...
      const pixPayment =
        paymentMethod === "pix" ? generatePixPayment() : undefined;
      ...
      const orders = JSON.parse(
        localStorage.getItem("gullagulla-orders") || "[]"
      );
      orders.push(order);
                  ...
                  <div className="bg-white p-6 border rounded-lg">
                    ...
                    <div className="space-y-3">
                      <p className="text-sm font-semibold">
                        Valor: {formatPrice(getGrandTotal())}
                      </p>
              ...
              {step === "success" && (
                <div className="text-center space-y-4">
                  ...
                  <p className="text-gray-600">
                    Para que possamos receber seu pedido, é necessário enviar a
                    mensagem de confirmação e o comprovante de pagamento pelo
                    WhatsApp.
                    <br />
                    <span className="font-semibold text-orange-600">
                      Clique no botão abaixo para enviar seu pedido e
                      comprovante!
                    </span>
                  </p>
                      ...
                      <a
                        href={`https://wa.me/5521976958970?text=${encodeURIComponent(
                          `NOVO PEDIDO - ${currentOrder.id}\n\n` +
                            ...
                            (currentOrder.customer.shipping.complement
                              ? `${currentOrder.customer.shipping.complement}\n`
                              : "") +
                            ...
                            currentOrder.items
                              ...
                              .join("\n") +
                            `\n\nResumo:\n` +
                            `Subtotal: R$ ${currentOrder.total
                              .toFixed(2)
                              .replace(".", ",")}\n` +
                            `Frete: ${
                              currentOrder.shipping === 0
                                ? "GRÁTIS"
                                : `R$ ${currentOrder.shipping
                                    .toFixed(2)
                                    .replace(".", ",")}`
                            }\n` +
                            `Total: R$ ${currentOrder.grandTotal
                              .toFixed(2)
                              .replace(".", ",")}\n\n` +
                            `Pagamento: ${
